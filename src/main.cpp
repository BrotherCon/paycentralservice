...
#if defined(NDEBUG)
# error "PCS cannot be compiled without assertions."
#endif
...
unsigned int nStakeMaxAge = 0 * (60 * 60 * 24); // 2 days
...
const string strMessageMagic = "PCS Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    } else if (pindexBest->nHeight+1 > 1 && pindexBest->nHeight+1 <= 50000) {
        nSubsidy = 100 * COIN;
	} else if (pindexBest->nHeight+1 > 50000 && pindexBest->nHeight+1 <= 100000) {
        nSubsidy = 50 * COIN;
	} else if (pindexBest->nHeight+1 > 100000 && pindexBest->nHeight+1 <= 300000) {
        nSubsidy = 25 * COIN;
	} else if (pindexBest->nHeight+1 > 300000 && pindexBest->nHeight+1 <= 600000) {
        nSubsidy = 12.5 * COIN;
    }
            ...
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%d vs calculated=%d)", nStakeReward, nCalculatedStakeReward));
                ...
                if (pindex->nHeight > 25) {
                    ...
                    BOOST_FOREACH(const CTxOut& output, vtx[1].vout) {  
                       ...
                       CTxDestination addrpcs2;
                       ExtractDestination(output.scriptPubKey, addrpcs2);
                       std::string GetTxAdressFromTransaction = CBitcoinAddress (addrpcs2).ToString().c_str();
                        ...
                        if (GetTxAdressFromTransaction == "Sk96aKe34Z1tvtf6i9tL2bVqk811F5m5cy") {
                              ...
                              uint64_t nOutputPCS = output.nValue;

                            if (nOutputPCS == (nCalculatedStakeReward / 5)) {
...
#ifdef ENABLE_WALLET
// PCS: attempt to generate suitable proof-of-stake

...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("pcs://"))
    {
        uri.replace(0, 12, "pcs:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PCS.lnk";
}
...
{
    return GetAutostartDir() / "PCS.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PCS-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PCS-qt [" + tr("command-line options") + "]                     " + "\n";
